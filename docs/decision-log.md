...
| 2026-09 | CI answer checks use Qwen2.5-1.5B | GitHub runs llama.cpp on CPU. Retrieval is all 152 rows. Answers are the rows flagged `ci` (24, every kind, all conflict and injection rows). Local eval uses the 3B GGUF | The 1.5B model can miss an answer the 3B gets. The floors are not lowered for that |
